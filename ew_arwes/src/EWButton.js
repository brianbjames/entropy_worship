...
const Button = memo((props: ButtonProps): JSX.Element => {
  const {visibility = 'visible', className, color = 'primary', zIndex= 100, variant = 'fill', animated, children, onClick } = props;

  const bleeps = useBleeps();
  const frameRef = useRef(null);

  useFrameSVGAssemblingAnimation(frameRef);

  return (
    <Animated
      as="button"
      style={{ zIndex: props.zIndex, visibility: 'visible' }}
    ...
    >
      ...
      <FrameSVGOctagon elementRef={frameRef} style={{ zIndex: 100 }} squareSize={theme.spacen(2)} />
...
.button {
  ...
  .button-back {
    ...
    z-index: 5; /* Ensure it's behind the content but above other layers */
  }
  
  .button-content {
    position: relative; /* Ensure content is on top */
    z-index: 100; /* Match or exceed the button's z-index */
    display: flex;
    flex-direction: row;
    ...
    <BleepsProvider {...bleepsSettings}>
      <Animator active={active}>
        <div style={{ margin: '1rem' }}>
          <Button color="secondary" variant="fill" style={{ zIndex: 100 }} style="visibility: 'visible'">
