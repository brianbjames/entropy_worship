...
function handlePong({ t0, t1, t2 }) {
  ...
  document.getElementById("latency").textContent =
    `you ${Math.round(rttMs / 2)} ms`;
  document.getElementById("clock-offset").textContent =
    `Δ${Math.round(serverTimeOffset)} ms`;
...
function startSequencer(epoch) {
  ...
  epochMs = epoch;
  lastScheduledIdx =
    Math.floor((Date.now() + serverTimeOffset - epochMs) / stepDurationMs()) -
    1;
  schedulerTick();
        ...
        bar.innerHTML = msg.peers
          ...
          .map((p) => {
            ...
            const cls = [
              "peer",
              p.id === myClientId ? "me" : "",
              ms != null && ms > 150 ? "peer-hi" : "",
            ]
              .filter(Boolean)
              .join(" ");
            const label = p.id === myClientId ? "you" : `p${p.id}`;
            const latStr =
              ms != null && p.id !== myClientId
                ? `<span class="peer-rtt"> ${ms}ms</span>`
                : "";
...
function initUI() {
  ...
  document
    .getElementById("rec-stop-btn")
    .addEventListener("click", stopRecording);
  document
    .getElementById("rec-export-btn")
    .addEventListener("click", exportMid);
}
...
function populateMidiDevices(access) {
  ...
  if (prevVal && [...access.inputs.keys()].includes(prevVal))
...
document
  .getElementById("midi-output-device")
  ...
  switch (status) {
    ...
    case 0xf8: {
      ...
      if (clockTickTimes.length >= 4) {
        const span =
          clockTickTimes[clockTickTimes.length - 1] - clockTickTimes[0];
        const bpm = Math.round(
          (((clockTickTimes.length - 1) / span) * 60000) / 24,
        );
    ...
    case 0xff:
      logMidiRow("Sys", null, "Reset", "", remote);
      break;
  }
}
...
function renderCCViz() {
  ...
  if (!el) return;
  const entries = Object.values(ccValues).sort((a, b) =>
    a.ch !== b.ch ? a.ch - b.ch : a.cc - b.cc,
  );
  if (entries.length === 0) {
    el.innerHTML =
      '<span class="muted" style="font-size:0.78rem">no CC received yet</span>';
    return;
  }
  el.innerHTML = "";
...
function renderPgmViz() {
  ...
  if (!el) return;
  const entries = Object.entries(pgmValues).sort((a, b) => +a[0] - +b[0]);
  if (entries.length === 0) {
...
function renderBendViz() {
  ...
  if (!el) return;
  const entries = Object.entries(bendValues).sort((a, b) => +a[0] - +b[0]);
  if (entries.length === 0) {
...
function vkbOn(note) {
  ...
  document
    .querySelectorAll(`.vkey[data-note="${note}"]`)
...
function recordEvent(bytes) {
  if (recording)
    recEvents.push({ ms: Date.now() - recStart, bytes: [...bytes] });
}
