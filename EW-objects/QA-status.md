# EW/OBJECTS QA CHECKLIST

41 objects. Manual browser testing. Mark each cell: PASS / FAIL / SKIP / N/A.
...
| # | Test | Status | Issues |
|---|------|--------|--------|
| AI1 | Source selector shows MIC and SCREEN / TAB AUDIO | | |
| AI2 | Unsupported sources disabled with warning (Safari: screen disabled) | | |
| AI3 | ENABLE AUDIO INPUT button requests permission | | |
| AI4 | MIC mode: browser permission prompt appears | | |
| AI5 | MIC mode: audio captured, scope shows waveform | | |
| AI6 | MIC mode: device selector lists available inputs | | |
| AI7 | MIC mode: switching device changes input source | | |
| AI8 | MIC mode: REFRESH button re-enumerates devices | | |
| AI9 | SCREEN mode: browser picker appears (Chrome/Edge only) | | |
| AI10 | SCREEN mode: tab audio captured and visualized | | |
| AI11 | SCREEN mode: no-audio warning if user doesn't share audio | | |
| AI12 | Input gain slider adjusts pre-amplification (0-2x) | | |
| AI13 | Output gain slider adjusts level without killing signal | | |
...
| AI19 | Level meter bar tracks input amplitude | | |
| AI20 | Peak hold indicator appears and decays | | |
| AI21 | dB readout updates in real-time | | |
| AI22 | Power toggle starts/stops capture | | |
| AI23 | Output mode selector: CONTROL / AUDIO / BOTH | | |
...
| # | Test | Status | Issues |
|---|------|--------|--------|
| N1 | Power toggle starts/stops noise | | |
| N2 | White noise type produces broadband hiss | | |
| N3 | Pink noise type produces lower-frequency noise | | |
| N4 | Brown noise type produces low rumble | | |
| N5 | Type selector switches noise color | | |
| N6 | Gain slider controls output level | | |
| N7 | Output mode: AUDIO / CONTROL / BOTH | | |
| N8 | Room input for gain modulates level | | |
| N9 | Room input for type changes noise color | | |
| N10 | Scope shows noise waveform | | |

### 3. OSCILLATOR — `oscillator.html`

| # | Test | Status | Issues |
|---|------|--------|--------|
| O1 | Power toggle starts/stops oscillator | | |
| O2 | Frequency slider changes pitch audibly | | |
| O3 | Detune slider shifts pitch in cents | | |
| O4 | Gain slider controls volume | | |
| O5 | Waveform selector: sine / triangle / sawtooth / square | | |
| O6 | Each waveform sounds distinct | | |
| O7 | Output mode: AUDIO / CONTROL / BOTH | | |
| O8 | Room input for frequency: modulates pitch | | |
| O9 | Room input for frequency: FM synthesis works (link another osc) | | |
...
| O14 | Scope shows waveform shape | | |
